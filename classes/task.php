...
if ( !class_exists( 'AZTimeTracker\\Task' ) ){
   ...
   class Task extends Base{
      ...
      public static $cpt_names = [ 'single' => 'az-task', 'plural' => 'az-tasks', 'uc_single' => 'Task', 'uc_plural' => 'Tasks' ];
      ...
      public function timeslots_box(){
         ...
         if ( $slots && !is_wp_error( $slots ) ){
            ...
            $this->print_timeslots_table( $rows, $total_total );  
         }
         else { 
      ...
      protected function print_timeslots_table( $rows, $total_total ){
     ...
     function columns_content( $column_name, $post_id ) {
         ...
         switch( $column_name ){
            ...
            case 'time':
               ...
               if ( $time )
                  echo "<span >".sprintf( "%02d%s%02d%s", floor( $time/3600 ), 'h', ( $time/60 )%60, 'm' )."</span><br> ( <span class='time-span'>".number_format( $time/3600,2 )."</span> )";
               break;
            case 'last_date':
               if ($this->last_date)
      ...
      function list_filter(){
         ...
         $this->print_button( $start, $end, $title );
      ...
      public function filter_list( $query ){
			...
			if ( isset($query->query[ 'post_type' ] ) && $query->query[ 'post_type' ] == 'az-task' && is_admin() && $pagenow=='edit.php' && $query->is_main_query() ){
            // filter by workspace
